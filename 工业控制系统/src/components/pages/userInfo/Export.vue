<template>
    <div class="w-full h-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
            <div class="absolute -top-40 -right-40 w-80 h-80 bg-gradient-to-br from-gray-600/10 to-gray-500/10 rounded-full blur-3xl animate-pulse"></div>
            <div class="absolute -bottom-40 -left-40 w-80 h-80 bg-gradient-to-br from-gray-700/10 to-gray-600/10 rounded-full blur-3xl animate-pulse" style="animation-delay: 2s;"></div>
        </div>

        <div class="relative w-full h-full flex gap-6 p-6">
            <!-- 左侧摄像头区域 -->
            <div class="h-full w-1/2">
                <div class="w-full h-full flex flex-col p-6 bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-gray-600/20 hover:shadow-3xl transition-all duration-500">
                    <!-- 标题和提示 -->
                    <div class="mb-8 text-center relative">
                        ...
                        <div class="flex items-center justify-center mb-4">
                            ...
                            <h2 class="text-4xl font-bold bg-gradient-to-r from-gray-200 to-gray-400 bg-clip-text text-transparent">出场车牌识别</h2>
                        </div>

                        <div class="relative">
                            <p class="text-xl text-gray-300 font-medium">请将车辆停在摄像头前方</p>
                        </div>
                    </div>
                    <!-- 摄像头选择器 -->
                    <div class="mb-6">
                        <label class="flex items-center text-sm font-semibold text-gray-300 mb-3">
                        ...
                        <div class="relative">
                            <select
                                ...
                                @change="switchCamera"
                                class="w-full px-4 py-3 bg-gray-700/70 backdrop-blur-sm border-2 border-gray-600/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 transition-all duration-300 appearance-none cursor-pointer hover:border-orange-400/50 text-gray-200"
                            >
                    ...
                    <!-- 视频显示区域 -->
                    <div class="flex-1 relative bg-gradient-to-br from-gray-700 to-gray-800 rounded-2xl overflow-hidden shadow-inner border-2 border-gray-600/50">
                        <video
                            ref="videoRef"
                            autoplay
                            playsinline
                        ...
                        >
                            <div class="text-center text-red-300 max-w-sm mx-auto p-6">
                                <div class="w-20 h-20 bg-red-800/50 rounded-full flex items-center justify-center mx-auto mb-4">
                                    <svg class="w-10 h-10 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"></path>
                                    </svg>
                                </div>
                                <h3 class="text-xl font-bold mb-2">⚠️ 摄像头启动失败</h3>
                                <p class="text-sm text-red-300 mb-4 bg-red-900/30 p-3 rounded-lg border border-red-700/50">{{ error }}</p>
            ...
            <!-- 右侧区域 -->
               <div class="h-full w-1/2">
                   <div class="w-full h-full flex gap-3 p-4 bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-gray-600/20">
                       <!-- 左侧区域 -->
                       <div class="w-1/2 h-full flex flex-col gap-3">
                           <!-- 车辆信息和停车时间合并卡片 - 始终显示 -->
                           <div class="flex-1 bg-gradient-to-br from-gray-800 to-gray-700 rounded-xl shadow-lg p-3 border border-gray-600 overflow-hidden">
                               <!-- 车辆信息部分 -->
                               <div class="mb-4">
                                   ...
                                   <!-- 车牌号码显示 -->
                                   <div class="bg-gray-700 rounded-lg p-2 mb-2 shadow-inner border-2 border-dashed border-gray-500">
                                       <div class="text-center">
                                           <div class="text-xl font-bold tracking-wider font-mono" :class="vehicleInfo.plateNumber ? 'text-blue-300' : 'text-gray-400'">
                                   ...
                                   <div class="grid grid-cols-2 gap-2">
                                       <div class="bg-gray-600/60 rounded-lg p-2">
                                           <span class="text-xs font-medium text-gray-300 block">车辆类型</span>
                                           <span class="text-sm font-bold" :class="vehicleInfo.vehicleType ? 'text-blue-300' : 'text-gray-400'">{{ vehicleInfo.vehicleType || '暂无数据' }}</span>
                                       </div>
                                       <div class="bg-gray-600/60 rounded-lg p-2">
                                           <span class="text-xs font-medium text-gray-300 block">停车位置</span>
                                           <span class="text-sm font-bold" :class="vehicleInfo.parkingSpace ? 'text-blue-300' : 'text-gray-400'">{{ vehicleInfo.parkingSpace || '暂无数据' }}</span>
                                       </div>
                                   </div>
                               </div>
                               ...
                               <div>
                                   <div class="flex items-center mb-2">
                                       ...
                                       <h3 class="text-lg font-bold text-white">停车时间</h3>
                                   </div>

                                   <div class="grid grid-cols-2 gap-2 mb-2">
                                       <div class="bg-gray-600/60 rounded-lg p-2">
                                           <span class="text-xs font-medium text-gray-300 block">入场时间</span>
                                           <span class="text-xs font-mono" :class="parkingTime.entryTime ? 'text-white' : 'text-gray-400'">{{ parkingTime.entryTime || '暂无数据' }}</span>
                                       </div>
                                       <div class="bg-gray-600/60 rounded-lg p-2">
                                           <span class="text-xs font-medium text-gray-300 block">出场时间</span>
                                           <span class="text-xs font-mono" :class="parkingTime.exitTime ? 'text-white' : 'text-gray-400'">{{ parkingTime.exitTime || '暂无数据' }}</span>
                                       </div>
                                   </div>

                                   <div class="bg-gray-700 rounded-lg p-2 shadow-inner border-2 border-dashed border-green-500">
                                       <div class="text-center">
                                           <div class="text-lg font-bold" :class="parkingTime.duration ? 'text-green-300' : 'text-gray-400'">{{ parkingTime.duration || '暂无数据' }}</div>
                                           <div class="text-xs text-gray-400">{{ parkingTime.durationEn || 'Parking Duration' }}</div>
                                       </div>
                                   </div>
                               </div>
                           </div>
                            ...
                            <!-- 费用计算卡片 - 只有识别到车牌后才显示 -->
                             <div v-if="vehicleInfo.plateNumber" class="flex-1 bg-gradient-to-br from-gray-800 to-gray-700 rounded-xl shadow-lg p-3 border border-gray-600 overflow-hidden">
                                <div class="flex items-center mb-2">
                                    ...
                                    <h3 class="text-lg font-bold text-white">费用计算</h3>
                                </div>

                                <div class="space-y-2">
                                    <div class="flex justify-between items-center bg-gray-600/60 rounded-lg p-2">
                                        <span class="text-xs font-medium text-gray-300">{{ feeCalculation.baseFee.label }}</span>
                                        <span class="text-sm font-bold text-yellow-300">{{ feeCalculation.baseFee.amount }}</span>
                                    </div>
                                    <div class="flex justify-between items-center bg-gray-600/60 rounded-lg p-2">
                                        <span class="text-xs font-medium text-gray-300">{{ feeCalculation.overtimeFee.label }}</span>
                                        <span class="text-sm font-bold text-yellow-300">{{ feeCalculation.overtimeFee.amount }}</span>
                                    </div>
                                    <div class="bg-gray-700 rounded-lg p-2 shadow-inner border-2 border-dashed border-yellow-500">
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm font-bold text-white">{{ feeCalculation.totalFee.label }}</span>
                                            <span class="text-xl font-bold text-yellow-300">{{ feeCalculation.totalFee.amount }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- 等待识别提示卡片 - 未识别到车牌时显示 -->
                            <div v-else class="flex-1 bg-gradient-to-br from-gray-800 to-gray-700 rounded-xl shadow-lg p-3 border border-gray-600 overflow-hidden flex items-center justify-center">
                        ...
                        <!-- 右侧扫码支付卡片 - 只有识别到车牌后才显示 -->
                             <div v-if="vehicleInfo.plateNumber" class="w-1/2 h-full bg-gradient-to-br from-gray-800 to-gray-700 rounded-xl shadow-lg p-4 border border-gray-600 overflow-hidden">
                            <div class="flex items-center mb-4">
                                ...
                                <h3 class="text-xl font-bold text-white">{{ paymentInfo.title }}</h3>
                            </div>

                            <!-- 二维码/支付成功区域 -->
                            <div class="bg-gray-700 rounded-lg p-8 mb-4 shadow-inner border-2 border-dashed" :class="isPaid ? 'border-green-500' : 'border-purple-500'">

                                <!-- 未支付状态 - 显示二维码 -->
                                <div v-if="!isPaid" class="text-center">
                                    <div class="w-48 h-48 bg-gray-600 rounded-lg mx-auto mb-4 flex items-center justify-center">
                                        <svg class="w-24 h-24 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path>
                                        </svg>
                                    </div>
                                    <div class="text-sm text-gray-300">{{ paymentInfo.qrCodeText }}</div>
                                </div>
                                ...
                                <div v-else class="text-center">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="text-sm text-gray-300">感谢您的使用，祝您出行愉快</div>
                                        <div class="text-xs text-gray-400">闸机将在3秒后自动开启</div>
                                    </div>
                                </div>
                            </div>
